    ...
    const FRVT::ReturnStatus &templGenRet)
{
    ...
    {
        candListStream << id << " " << i << " " << static_cast<underlying_type<ReturnCode>::type>(ret.code) << " " << candidate.isAssigned << " " << candidate.templateId << " "
                       << candidate.similarityScore << " " << (i == 0 ? decision : false) << endl;
        i++;
    }
}
